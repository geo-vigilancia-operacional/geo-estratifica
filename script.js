...
function atualizarProgramados() {
    ...
    const totais = {
        ...
        comercios: dadosQuadrasSelecionadas.reduce((acc, cur) => acc + getNumero(cur.C || 0), 0),
        terrenos: dadosQuadrasSelecionadas.reduce((acc, cur) => acc + getNumero(cur.TB || 0), 0),
        outros: dadosQuadrasSelecionadas.reduce((acc, cur) => acc + getNumero(cur.OU || 0), 0),
        pontosEstrategicos: dadosQuadrasSelecionadas.reduce((acc, cur) => acc + getNumero(cur.PE || 0), 0),
        apartamentos: dadosQuadrasSelecionadas.reduce((acc, cur) => acc + getNumero(cur['AP. ACIMA DO TÉRREO'] || 0), 0),
        habitantes: dadosQuadrasSelecionadas.reduce((acc, cur) => acc + getNumero(cur.HABITANTES || 0), 0),
    ...
    resumoProgramados.innerHTML = `
        ...
        <span><strong>Residências (R):</strong> ${totais.residencias}</span>
        <span><strong>Comércios (C):</strong> ${totais.comercios}</span>
        <span><strong>Terrenos Baldios (TB):</strong> ${totais.terrenos}</span>
        <span><strong>Outros (OU):</strong> ${totais.outros}</span>
        <span><strong>Pontos Estratégicos (PE):</strong> ${totais.pontosEstrategicos}</span>
        <span><strong>Apartamentos Acima Térreo:</strong> ${totais.apartamentos}</span>
        <span><strong>Total de Habitantes:</strong> ${totais.habitantes}</span>
        <span>🏠 <strong>Imóveis Programados:</strong> <span id="imoveisProgramadosValue">${imoveisProgramados}</span></span>
        <span>🐕 <strong>Cães:</strong> ${totais.caes}</span>
        <span>🐈 <strong>Gatos:</strong> ${totais.gatos}</span>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    observer.observe(document.getElementById("resumoProgramados"), {
        childList: true,
        subtree: true
    });
    atualizarQuadrasTrabalhadas();
